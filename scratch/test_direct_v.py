...
def test_pipeline(df_outage, init_lat, init_lon, init_spd, init_hdg, predictor, k_scale, direct_v=True):
    n = len(df_outage)
    dt = 0.1
    ...
    for k in range(n):
        ...
        if len(window_buffer) >= config.WINDOW_SIZE and (k % 5 == 0):
            ...
            if ev_class == 0 or pred_d < 0.1:
                ...
                if direct_v:
                    # Direct velocity measurement using calibrated displacement
                    ...
                    ukf.update_velocity_net(pred_d, sigma, window_dur=2.0)
...
for _, row in df_outages.iterrows():
    ...
    p_lat, p_lon, _ = test_pipeline(df_sub, init_lat, init_lon, init_spd, init_hdg, predictor, k_calib, direct_v=True)
    score = score_outage_segment(p_lat, p_lon, gt_lat, gt_lon, row["distance_travelled_m"], dur, gt_hdg)
    print(f"[Direct V] {dur:3d}s | Dist: {score['distance_m']:6.1f}m | FPE: {score['fpe_m']:5.2f}m | Drift: {score['drift_percent']:5.2f}% | Along: {score['along_track_rms_m']:5.2f}m | Cross: {score['cross_track_rms_m']:5.2f}m")
